...
resource "aws_docdb_cluster" "main" {
  ...
  engine                          = "docdb"
  master_username                 = jsondecode(aws_secretsmanager_secret_version.secret.secret_string)["DOCDB_USERNAME"]
  master_password                 = jsondecode(aws_secretsmanager_secret_version.secret.secret_string)["DOCDB_PASSWORD"]
